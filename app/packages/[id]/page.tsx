...
export default async function PackageDetailPage({ params }: PageProps) {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#060709]/95 backdrop-blur-2xl text-[#f4f0e8] pt-28 sm:pt-32 pb-20 sm:pb-28 px-4 sm:px-8 md:px-12 flex items-center justify-center relative overflow-y-auto">
        
        {/* Dynamic Modal Window with Generous Spacing & Padding */}
        <div className="relative w-full max-w-3xl bg-[#0c0e13] border border-[#232630] hover:border-[var(--gold-border)] rounded-2xl shadow-[0_30px_90px_rgba(0,0,0,0.95)] p-6 sm:p-10 md:p-12 my-6 sm:my-10 transition-all duration-300">
          
          {/* Top Bar with Number & Close Button */}
          <div className="flex flex-wrap justify-between items-center pb-6 border-b border-[#1f222a] mb-8 gap-4">
            <div className="flex items-center gap-3">
              <span className="font-mono text-xs tracking-widest text-[var(--gold)] uppercase font-semibold px-3 py-1 rounded bg-[var(--gold)]/10 border border-[var(--gold)]/20">
                {pkg.num}
              </span>
            ...
            <Link
              href="/#packages"
              className="group flex items-center gap-2 font-mono text-xs tracking-widest text-[#a0a4b0] hover:text-white uppercase transition-colors px-4 py-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/10"
          ...
          <div className="mb-8">
            <h1 className="font-bebas text-4xl sm:text-5xl md:text-6xl text-white tracking-wider leading-none mb-3">
              {pkg.name}
            </h1>
            <div className="font-cormorant italic text-lg sm:text-xl text-[var(--gold)]">
              {pkg.subtitle}
            </div>
          </div>

          {/* Description Block */}
          <div className="p-5 sm:p-6 mb-10 border-l-4 border-[var(--gold)] bg-[#131620] rounded-r-xl">
            <p className="font-cormorant text-lg sm:text-xl text-[#e0deda] leading-relaxed">
              {pkg.description}
            </p>
          </div>

          {/* Pricing & Milestone Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10 bg-[#11141c] p-6 sm:p-7 rounded-xl border border-white/10 shadow-inner">
            <div className="p-2">
              <div className="font-mono text-[10px] tracking-widest text-[var(--muted)] uppercase mb-2">
                ESTIMATED INVESTMENT RANGE
              </div>
              <div className="font-bebas text-3xl sm:text-4xl text-[var(--gold)] tracking-wide">
                {pkg.range}
              </div>
            </div>
            <div className="p-2 border-t sm:border-t-0 sm:border-l border-white/10 pt-4 sm:pt-0 sm:pl-6">
              <div className="font-mono text-[10px] tracking-widest text-[var(--muted)] uppercase mb-2">
                PAYMENT MILESTONE SCHEDULE
              </div>
              <div className="font-mono text-base font-bold text-white mb-1.5">
                {pkg.paymentSplit}
              </div>
          ...
          {/* Base Inclusion */}
          <div className="mb-8 p-4 px-6 bg-[#12151e] rounded-lg border border-white/5">
          ...
          <div className="space-y-6 sm:space-y-8 mb-10">
            {pkg.pillars.map((pillar, idx) => (
              <div
                key={idx}
                className="p-6 sm:p-7 rounded-xl bg-[#141722] border border-white/10 hover:border-[var(--gold-border)] transition-all duration-300 shadow-md"
              >
